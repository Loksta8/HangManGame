...
def Read(aFilename, aNumLetters, aWordsFound):
    with io.open(aFilename, 'rt') as readfile:
        for line in readfile:
            line = line.rstrip() # strip white space
            if(len(line) >= aNumLetters):
                aWordsFound.append(line) # Words have been found greater than or
                                         # equal to the argument given
        if not aWordsFound:
...
def main(argv):
    wordsFound = []
    filename = "./wordlist_dictionary.txt" # Relative path to word list file
    numLetters = int(argv) # The number up to this amount of letters we will filter for
    Read(filename, numLetters, wordsFound) # Read the file
    print("Would you like to see the words? " , "\n")
